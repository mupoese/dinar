...
#if defined(NDEBUG)
# error "Dinar cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf12e3438b7dfc7b0aa5440eb7df24e853c6ba1fdb2c6a1f05647f9daca79a702");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dinar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dinar Signed Message:\n";
...
{
    // Dinar: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 708060 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 708060); // Dinar: 708k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Dinar: 7 days
static const int64 nTargetSpacing = 9 * 60; // Dinar: 9 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dinar: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dinar: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb9a13c97ba0954681c82ca15a14b9141077e0f78ad186a065ad0d40f943bb6c6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Donald Trump fucks up the World!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 35 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046a671e3e4387eb58f628e534e753309a93fd446470cc3fe746793ec8d2b913581ce8a904408604aa46b609ad3770dece371a57eb1094f6423b02050e5b82ba68") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516711276;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084957899;

        if (fTestNet)
        {
            block.nTime    = 1516711276;
            block.nNonce   = 386837708;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5855c092f0ffc0debfe7930ce216bf0b84f301216203ac0f05f8e0544bf9abeb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xcd, 0xbe, 0xd6 }; // Dinar: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DinarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DinarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DinarMiner(CWallet *pwallet)
{
    printf("DinarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dinar-miner");
    ...
    try { loop {
        ...
        printf("Running DinarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DinarMiner, pwallet));
}
